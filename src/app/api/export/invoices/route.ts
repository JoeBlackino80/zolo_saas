...
export async function GET(req: NextRequest) {
  ...
  const rl = await rateLimit(`export-invoices:${user.id}`, 20, 3600_000);
  if (!rl.allowed) return NextResponse.json({ error: 'Too many exports' }, { status: 429, headers: { 'Retry-After': String(Math.ceil(rl.resetIn / 1000)) } });
