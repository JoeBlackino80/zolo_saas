    ...
    <div className="p-4 sm:p-8 max-w-4xl">
      ...
      <Card className="mb-4 bg-gradient-to-br from-slate-900 to-slate-800 text-white border-none">
        <div className="p-6 flex items-center justify-between">
          <div>
            <div className="text-xs uppercase tracking-wider text-slate-400 font-semibold">Prihlásený účet</div>
            <div className="text-2xl font-bold mt-1">{user.email}</div>
            <div className="text-xs text-slate-400 font-mono mt-1">User ID: {user.id}</div>
            <div className="flex gap-6 mt-4 text-sm">
              <div>
                <div className="text-xs text-slate-400">Vytvorený</div>
                <div className="font-medium">{fmtDate(user.created_at)}</div>
              </div>
              <div>
                <div className="text-xs text-slate-400">Posledné prihlásenie</div>
                <div className="font-medium">{fmtDate(user.last_sign_in_at || '')}</div>
              </div>
              <div>
                <div className="text-xs text-slate-400">AAL</div>
