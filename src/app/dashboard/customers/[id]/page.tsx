    ...
    <div className="p-4 sm:p-8 max-w-5xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <Card><div className="p-5">
          <div className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Tržby s týmto klientom</div>
          <div className="text-2xl font-bold mt-2">{fmtEur(totalRevenue)}</div>
          <div className="text-xs text-zinc-500 mt-1">{rows.filter((i) => i.type === 'invoice').length} faktúr</div>
        </div></Card>
        <Card><div className="p-5">
          <div className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Zaplatené</div>
          <div className="text-2xl font-bold mt-2 text-emerald-600">{fmtEur(totalPaid)}</div>
        </div></Card>
        <Card><div className="p-5">
          <div className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Po splatnosti</div>
      ...
      <Card className="mb-4">
        ...
        <div className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
          {contact.email && <div className="flex items-center gap-2"><Mail size={14} className="text-zinc-400" /><a href={`mailto:${contact.email}`} className="text-zinc-900 hover:underline">{contact.email}</a></div>}
          {contact.phone && <div className="flex items-center gap-2"><Phone size={14} className="text-zinc-400" /><a href={`tel:${contact.phone}`} className="text-zinc-900 hover:underline">{contact.phone}</a></div>}
          {contact.web && <div className="flex items-center gap-2"><Globe size={14} className="text-zinc-400" /><a href={contact.web} target="_blank" rel="noopener" className="text-zinc-900 hover:underline">{contact.web}</a></div>}
          <div><span className="text-zinc-500">DIČ:</span> <span className="font-mono">{contact.dic || '—'}</span></div>
          <div><span className="text-zinc-500">IČ DPH:</span> <span className="font-mono">{contact.ic_dph || '—'}</span></div>
          <div><span className="text-zinc-500">Adresa:</span> {[contact.street, contact.city, contact.zip].filter(Boolean).join(', ') || '—'}</div>
        </div>
      </Card>
      ...
      <Card>
        ...
        {rows.length === 0 ? (
          <div className="p-8 text-center text-sm text-zinc-500">Žiadne faktúry s týmto klientom</div>
        ) : (
