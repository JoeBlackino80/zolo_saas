                  ...
                  {rows.map((r) => {
                    ...
                    return (
                      <tr key={r.id} className="hover:bg-zinc-50">
                        <td className="px-5 py-3 text-zinc-700 text-sm">{companyName || '—'}</td>
                        <td className="px-3 py-3 font-mono text-xs">{r.number}</td>
                        <td className="px-3 py-3 text-zinc-700">{r.customer_name || '—'}</td>
...
function Kpi({ label, value, note, variant }: { label: string; value: string; note: string; variant?: 'red' | 'green' | 'amber' }) {
  const color = variant === 'red' ? 'text-red-600' : variant === 'green' ? 'text-emerald-600' : variant === 'amber' ? 'text-amber-600' : 'text-zinc-900';
  return (
    <Card>
      <div className="p-5">
...
function Bucket({ label, count, amount, variant }: { label: string; count: number; amount: number; variant?: 'red' | 'green' | 'amber' }) {
  ...
  return (
    <div className="bg-white p-4 text-center">
      ...
      <div className="text-xs text-zinc-500 mt-1">{count} faktúr</div>
    </div>
  );
}
