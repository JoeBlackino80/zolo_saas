  ...
  return (
    <div className="p-4 sm:p-8 max-w-7xl">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <Card>
          <div className="p-5">
            <div className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Spolu povinnosť</div>
        ...
        <Card>
          <div className="p-5">
            <div className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Firmy platia</div>
            <div className="text-2xl font-bold mt-2">{rows.filter((r) => r.t.obligation > 0.01).length}</div>
          </div>
        </Card>
        <Card>
          <div className="p-5">
            <div className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Firmy s nárokom</div>
              ...
              {rows.map((r) => {
                ...
                return (
                  <tr key={r.company_id} className="hover:bg-zinc-50">
                    <td className="px-4 py-2.5 font-sans text-sm font-medium text-zinc-900">{r.company_name}</td>
                    ...
                    <td className={`text-right px-4 py-2.5 font-bold ${obl > 0.01 ? 'text-red-600' : obl < -0.01 ? 'text-emerald-600' : 'text-zinc-400'}`}>
                      {fmtEur(obl)}
                    </td>
                  </tr>
                );
      ...
      <div className="mt-4 flex gap-3 text-sm">
        <Badge variant="red">Červené</Badge> <span className="text-zinc-500">platíš DPH</span>
        <Badge variant="green">Zelené</Badge> <span className="text-zinc-500">nárok na vrátenie</span>
      </div>
    </div>
  );
}
