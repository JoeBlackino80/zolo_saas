...
export default function LoginPage() {
  ...
  async function handleGoogleOAuth() {
    setLoading(true);
    setStatus({ msg: 'Presmerúvam na Google…', kind: 'muted' });
    ...
    if (error) {
      setStatus({ msg: 'Google OAuth chyba: ' + error.message, kind: 'error' });
      setLoading(false);
    }
    // Ak úspech, browser sa presmeruje na Google — netreba nič ďalej.
  }
    ...
    <div className="min-h-screen flex items-center justify-center p-6 bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-950 text-zinc-200">
      ...
      <div className="relative w-full max-w-md bg-zinc-900/70 backdrop-blur-xl border border-white/10 rounded-2xl p-9 shadow-2xl">
        ...
        {requiresMfa ? (
          ...
          <>
            {/* Google OAuth tlačidlo — spoločné pre login aj signup */}
            <button
              type="button"
              onClick={handleGoogleOAuth}
              disabled={loading}
              className="w-full py-3 bg-white text-zinc-900 font-medium rounded-full hover:bg-zinc-100 transition-colors disabled:opacity-50 flex items-center justify-center gap-2 mb-3"
            >
              ...
              {mode === 'login' ? 'Prihlásiť sa cez Google' : 'Registrovať cez Google'}
            </button>

            {/* Divider */}
            <div className="flex items-center gap-3 mb-3">
              <div className="flex-1 h-px bg-white/10" />
              <span className="text-[10px] uppercase tracking-wider text-zinc-500 font-semibold">alebo email</span>
              <div className="flex-1 h-px bg-white/10" />
            </div>
            ...
            {mode === 'login' ? (
              <form onSubmit={handleLogin} className="space-y-3">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-3 bg-white/10 border border-white/20 text-white font-medium rounded-full hover:bg-white/20 transition-colors disabled:opacity-50"
                >
                  Prihlásiť sa
                </button>
